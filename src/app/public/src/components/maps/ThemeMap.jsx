...
export default class ThemeMap extends React.Component {
  ...
  componentDidMount = () => {
    ...
    L.control
      .defaultExtent({ position: "topright", text: "", title: "Zoom to Texas" })
      .addTo(this.map);
    ...
    const baseLayer = L.tileLayer(
      constants.BASE_MAP_LAYER.url,
      constants.BASE_MAP_LAYER.options
    );
    map.addLayer(baseLayer);

    const countiesUrl = CdbUtil.createCountiesLayer();
    const countiesLayer = (this.countiesLayer = L.tileLayer(
      countiesUrl.tilesUrl
    ));
    const countiesLabelsUrl = CdbUtil.createCountiesLabelsLayer();
    const countiesLabelsLayer = (this.countiesLabelsLayer = L.tileLayer(
      countiesLabelsUrl.tilesUrl
    ));
  ...
  updateMap = (props) => {
    ...
    const entityFeatures = R.map((group) => {
      ...
      const entityProperties = R.assoc(
        "ValueSum",
        valueSum,
        R.pick(
          R.concat(["EntityId", "EntityName", "ValueSum"], npdProps),
          entity
        )
      );
      ...
      pointToLayer: (feat, latlng) => {
        ...
        if (minVal === maxVal) {
          ...
          radius = scale(
            ...
            minVal,
            maxVal,
            constants.MIN_ENTITY_POINT_RADIUS,
            constants.MAX_ENTITY_POINT_RADIUS
          );
        }
    ...
    if (props.boundary && !R.isEmpty(props.data.rows)) {
      this.boundaryLayer = L.geoJson(props.boundary, {
        ...
        pointToLayer: (feature, latlng) => {
          // if boundary is statewide centroid, centroid is invisible
    ...
    if (props.theme === "strategies" && props.projects != undefined) {
      ...
      const projectFeatures = R.map((prj) => {
        const displayCost = prj.CapitalCost
          ? "$" +
            prj.CapitalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
          : "n/a";
  ...
  displaySourceLayer = (props, bounds) => {
    if (props.theme === "supplies" || props.theme === "strategies") {
      const hasValue = props.data.rows.filter(
        (record) => record[`Value_${props.decade}`] > 0
      );
      ...
      if (sources.length != 0) {
        CdbUtil.getSource(sources).then((results) => {
          ...
          // limit mappable features to those that fall within mappableSourceTypes
          // and which also are not new features.
          // this eliminates centroids added superficially to represent
          // statewide source types.
    ...
    return (
      <div>
        ...
        {this.props.theme === "strategies" && (
          <p className="note">
            Red triangles indicate capital projects associated with strategy
            supplies that have been assigned to a Water User Group.{" "}
            <a className="pointerHover" onClick={this.toggleProjects}>
